...
module type T = sig
  ...
  *)
  ...
  (** [check ra n]
      ...
      @return
        [true] if the [n]th cell of [ra] is full, [false] if it is empty. Note
        that even if [check ar n] returns [true], a subsequent {!get}[ar n] can
        return [None]. *)
  ...
  (** [screate s n el]
      ...
      @return a resizable array of length [n] using strategy [s]. *)
  ...
  *)
  ...
  (** [sof_std s ar] converts a standard weak array to a resizable one, using
      strategy [s]. *)

  val of_std : 'a Weak.t -> 'a t
  (** [of_std ar] converts a standard weak array to a resizable one using the
      default strategy. *)
