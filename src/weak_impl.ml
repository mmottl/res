...
module Make (S : Strat.T) = struct
  ...
  let screate_fresh strategy n =
    let res = { ar = creator 0; vlix = n - 1; strategy } in
  ...
  let sempty strategy =
    let res = { ar = creator 0; vlix = -1; strategy } in
  ...
  let unsafe_blit_on_other ra1 ofs1 ra2 ofs2 len =
    ...
    for i = ofs1 to ofs1 + len - 1 do
      unsafe_set ra2 (i + ofs_diff) (unsafe_get ra1 i)
    done
  ...
  let unsafe_blit ra1 ofs1 ra2 ofs2 len =
    ...
    if ofs1 < ofs2 then
      for i = len - 1 downto 0 do
        unsafe_set ra2 (ofs2 + i) (unsafe_get ra1 (ofs1 + i))
      done
    else
      for i = 0 to len - 1 do
        unsafe_set ra2 (ofs2 + i) (unsafe_get ra1 (ofs1 + i))
  ...
  let fold_right f ra accu =
    ...
    for i = ra.vlix downto 0 do
      res := f (unsafe_get ra i) !res
    done;
    !res

  let rec for_all_aux i p ra =
    if i > ra.vlix then true
    else if p (unsafe_get ra i) then for_all_aux (i + 1) p ra
    else false
  ...
  let rec exists_aux i p ra =
    ...
    else if p (unsafe_get ra i) then true
    else exists_aux (i + 1) p ra

  let exists p ra = exists_aux 0 p ra

  let rec mem_aux i x ra =
    if i > ra.vlix then false
    else if unsafe_get ra i = x then true
    else mem_aux (i + 1) x ra
